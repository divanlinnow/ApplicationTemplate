...
namespace Domain.Services.Core.Tests
{
    ...
    public class CountryServiceTests
    {
        ...
        public void FindCountryById_Service_Success()
        {
            // Arrange
            countryService = new CountryService(mockRepository.Object, mockLogger.Object, mockCache.Object, mockTelemetry.Object);
            mockRepository.Setup(x => x.FindById<Country>(Guid.Empty)).Returns(new Country()).Verifiable();

            // Act
            var response = countryService.FindCountryById(Guid.Empty);

            // Assert
            Assert.IsNotNull(response);
            Assert.IsNotNull(response.Result);
            Assert.IsInstanceOfType(response, typeof(GenericServiceResponse<CountryDto>));
            mockRepository.Verify(x => x.FindById<Country>(Guid.Empty), Times.Once);
        }
        ...
        public void FindCountryById_Service_Fail()
        {
            // Arrange
            countryService = new CountryService(mockRepository.Object, mockLogger.Object, mockCache.Object, mockTelemetry.Object);
            mockRepository.Setup(x => x.FindById<Country>(Guid.Empty)).Throws(new Exception()).Verifiable();
        ...
        public void DeleteCountryById_Service_Success()
        {
            // Arrange
            countryService = new CountryService(mockRepository.Object, mockLogger.Object, mockCache.Object, mockTelemetry.Object);
            mockRepository.Setup(x => x.Delete<Country>(Guid.Empty)).Returns(true).Verifiable();

            // Act
            var response = countryService.DeleteCountry(Guid.Empty);
            ...
            Assert.IsFalse(response.Notifications.HasErrors());
            Assert.IsInstanceOfType(response, typeof(GenericServiceResponse<bool>));
            mockRepository.Verify(x => x.Delete<Country>(Guid.Empty), Times.Once);
        }
        ...
        public void DeleteCountryById_Service_Fail()
        {
            // Arrange
            countryService = new CountryService(mockRepository.Object, mockLogger.Object, mockCache.Object, mockTelemetry.Object);
            mockRepository.Setup(x => x.Delete<Country>(Guid.Empty)).Returns(false).Verifiable();
