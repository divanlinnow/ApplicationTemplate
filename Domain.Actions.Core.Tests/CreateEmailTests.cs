...
namespace Domain.Actions.Core.Tests
{
    ...
    public class CreateEmailTests
    {
        ...
        public void CreateEmail_Action_Success()
        {
            // Arrange
            ...
            mockClientServicesProvider.Setup(x => x.EmailService.CreateEmail(emailDto)).Returns(fakeResponse).Verifiable();

            var viewModel = new GenericViewModel();

            var action = new CreateEmail<GenericViewModel>(mockClientServicesProvider.Object)
            {
                OnComplete = model => viewModel = model
            };

            // Act
            var result = action.Invoke(emailDto);
